...
try {
    ...
    $stmt = $pdo->prepare("UPDATE product_subcategories SET status = 'archived' WHERE id = ?");
    $stmt->execute([$id]);
    
    // Audit log
    $newRecord = getRecordForAudit($pdo, 'product_subcategories', $id);
    logAudit($pdo, 'product_subcategories', $id, 'delete', $oldRecord, $newRecord, $user['id'] ?? null);
